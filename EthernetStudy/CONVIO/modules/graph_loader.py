...
def project_point_to_segment(p: Point, a: Point, b: Point) -> Tuple[Point, float]:
    ...
    """
    ax, ay, az = a # Unpack 3D point
    bx, by, bz = b # Unpack 3D point
    px, py, pz = p
    vx, vy, vz = bx - ax, by - ay, bz - az
    wx, wy, wz = px - ax, py - ay, pz - az
    v_len_sq = vx * vx + vy * vy + vz * vz
    if v_len_sq == 0.0:
        return a, 0.0 # Segment is a point, return 'a'
    t = (wx * vx + wy * vy + wz * vz) / v_len_sq
    t_clamped = max(0.0, min(1.0, t))
    qx = ax + t_clamped * vx
    qy = ay + t_clamped * vy
    qz = az + t_clamped * vz
    return (qx, qy, qz), t_clamped
...
class GraphLoader:
    ...
    def load_chassis_graph(self, json_file_path: str) -> nx.Graph:
        ...
        for nid in data["nodes"]:
            ...
            # Ensure coordinates are always 3D, defaulting Z to 0.0 if not provided
            x, y = float(coord[0]), float(coord[1])
            z = float(coord[2]) if len(coord) > 2 else 0.0
            G.add_node(nid, pos=(x, y, z), x=x, y=y, z=z, type="chassis", is_io=False)
    ...
    def load_io_coordinates_from_csv(self, csv_file_path: str) -> List[Point]:
        ...
        with open(csv_file_path, "r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                z = float(row["Z"]) if "Z" in row else 0.0 # Read Z if available, else default to 0.0
                pts.append((x, y, z))
        self.logger.info(f"Loaded {len(pts)} I/O points (3D)")
        return pts
    ...
    def add_io_nodes_to_graph(self, graph: nx.Graph, points: List[Point]) -> nx.Graph:
        ...
        chassis_positions = [G.nodes[n]["pos"] for n in chassis_nodes] # These are now 3D points
        kdtree = KDTree(chassis_positions)
        
        for idx, io_pos in enumerate(points, start=1):
            io_id = f"IO_{idx}"
            # Ensure I/O nodes are added with 3D positions
            G.add_node(io_id, pos=io_pos, x=io_pos[0], y=io_pos[1], z=io_pos[2], type="io", is_io=True)
            ...
            if dist > self.min_direct_node_distance_mm and self.allow_projection_on_edge:
                ...
                if best_proj:
                    ...
                    if 1e-6 < t < 1 - 1e-6:  # Ensure the projection is not at an endpoint.
                        ...
                        # Ensure projected nodes are added with 3D positions
                        G.add_node(proj_node, pos=q, x=q[0], y=q[1], z=q[2], type="chassis", is_io=False)
