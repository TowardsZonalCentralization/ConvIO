...
from PyQt5.QtGui import QFont, QPixmap, QColor, QMatrix4x4, QImage, QPainter 
...
class OptimizationWorker(QThread):
    ...
    def clustering_task(self):
        ...
        def _clustering_internal():
            ...
            linkage = self.kwargs.get('linkage_method', 'complete')  
...
class WiringHarnessOptimizer(QMainWindow):
    ...
    def _apply_config(self):
        ...
        self.gl_background_color = self._parse_color(gui_cfg.get("3d_background_color", "white"))
        self.gl_node_size_multiplier = float(gui_cfg.get("3d_node_size_multiplier", 0.002))


    def _make_solid_item(self, shape: str, center: Tuple[float, float, float],
        ...
        if shape == "sphere":
            # radius = size/2; smooth shading for a round sphere
            md = gl.MeshData.sphere(rows=16, cols=32, radius=size * 0.5)
            item = gl.GLMeshItem(meshdata=md, color=rgba, drawFaces=True,
                                 drawEdges=False, smooth=True, glOptions='opaque')
            item.translate(cx, cy, cz)
            return item

        if shape in ("box", "cube"):
            # cube built from triangles; flat shading (smooth=False)
            half = size * 0.5
            verts = np.array([
            ...
            verts += np.array([cx, cy, cz], dtype=float)
    ...
    def _create_visualization_panel(self) -> QWidget:
        ...
        # 3D Visualization (always enabled now)
        self.graph_view_3d = gl.GLViewWidget()
        self._setup_gl_view(self.graph_view_3d, 'Network Graph (3D)')
        self.tab_widget.addTab(self.graph_view_3d, "Network Graph (3D)")
        self.graph_legend_overlay = self._create_legend_overlay(self.graph_view_3d)

        self.elbow_widget = MatplotlibWidget()
        self.tab_widget.addTab(self.elbow_widget, "Elbow Analysis")
        
        self.hpc_view_3d = gl.GLViewWidget()
        self._setup_gl_view(self.hpc_view_3d, 'Overall Wiring (3D)')
        self.tab_widget.addTab(self.hpc_view_3d, "Overall Wiring (3D)")
        self.hpc_legend_overlay = self._create_legend_overlay(self.hpc_view_3d)

        self.cluster_view_3d = gl.GLViewWidget()
        self._setup_gl_view(self.cluster_view_3d, 'EEA with I/O extenders (3D)')
        self.tab_widget.addTab(self.cluster_view_3d, "EEA with I/O extenders (3D)")
        self.cluster_legend_overlay = self._create_legend_overlay(self.cluster_view_3d)
    ...
    def _setup_gl_view(self, view: gl.GLViewWidget, title: str):
        """Setup PyQtGraph OpenGL 3D view with common settings."""
        view.setWindowTitle(title)
        view.setBackgroundColor(self.gl_background_color) # Set background from config
        view.setCameraPosition(distance=10000, elevation=self.config.get("gui.3d_view_elevation", 30),
                               azimuth=self.config.get("gui.3d_view_azimuth", 45)) # Default camera position
        # Add a grid to the 3D view
        grid = gl.GLGridItem()
        view.addItem(grid)

    def _parse_color(self, color_str: str) -> QColor:
        """Parses a color string (hex, name) to QColor."""
        if color_str.startswith("#"):
            return QColor(color_str)
        return QColor(color_str.lower()) # PyQt handles common color names

    def _create_legend_overlay(self, parent_view: gl.GLViewWidget) -> QGraphicsView:
        """Creates and returns a QGraphicsView to be used as a legend overlay."""
        overlay_view = QGraphicsView(parent_view)
        overlay_view.setFixedSize(200, 150)
        overlay_view.setStyleSheet("background-color: rgba(255, 255, 255, 180); border: 1px solid black;")
        ...
        overlay_view.move(10, 10) 
        overlay_view.setScene(QGraphicsScene(overlay_view))
        overlay_view.hide() 
        return overlay_view

    def _update_overlay_legend(self, overlay_view: QGraphicsView, legend_items: List[Tuple[str, QColor]]):
        """Updates the content of a legend overlay with new items."""
        scene = overlay_view.scene()
        scene.clear() 

        y_offset = 5
        for text, color in legend_items:
            # Color square
            color_rect = QGraphicsRectItem(0, y_offset, 20, 20)
            color_rect.setBrush(color)
            color_rect.setPen(QColor(Qt.black))
            scene.addItem(color_rect)

            # Text label
            text_item = QGraphicsTextItem(text)
            text_item.setFont(QFont("Arial", 10))
            text_item.setPos(25, y_offset)
            scene.addItem(text_item)
            
            y_offset += 25

        # Adjust overlay size based on content
        new_height = len(legend_items) * 25 + 30
        overlay_view.setFixedSize(200, new_height)
        overlay_view.show()

    def _on_tab_changed(self, index: int):
        """Handles tab changes to update legend visibility and content."""
        # Hide all legends first
        self.graph_legend_overlay.hide()
        self.hpc_legend_overlay.hide()
        self.cluster_legend_overlay.hide()

        # Re-draw the content of the active 3D tab to ensure its legend is updated
        current_widget = self.tab_widget.widget(index)
        if current_widget == self.graph_view_3d:
            if self.current_graph:
                self._visualize_graph_3d(self.current_graph)
        elif current_widget == self.hpc_view_3d:
            if self.hpc_results and self.current_graph:
                self._visualize_hpc_wiring(is_3d=True)
        elif current_widget == self.cluster_view_3d:
            if self.clustering_results and self.current_graph:
                self._visualize_clustering_results(self.cluster_view_3d, self.clustering_results, is_3d=True)
    ...
    def on_graph_loaded(self, results):
        ...
        # Update visualization (now only 3D)
        self._visualize_graph_3d(self.current_graph)
    ...
    def on_full_analysis_completed(self, results):
        ...
        if best_method and best_method in results.get('results', {}):
            ...
            self.on_clustering_completed(best_result_data) 
        else:
    ...
    def on_clustering_completed(self, results):
        ...
        # Visualize clustering results (now only 3D)
        self._visualize_clustering_results(self.cluster_view_3d, self.clustering_results, is_3d=True)
        if self.hpc_results:
    ...
    def on_hpc_completed(self, results):
        ...
        if self.hpc_results:
            ...
            # Visualize HPC wiring (now only 3D)
            self._visualize_hpc_wiring(is_3d=True)
    ...
    def _visualize_graph(self, graph):
        """This function is deprecated as we are moving to 3D only."""
        self.log("2D graph visualization is deprecated.")
        return


    def _visualize_graph_3d(self, graph):
        """Visualize the network graph in 3D."""
        self.graph_view_3d.clear()
        pos = self._get_node_positions(graph)

        if not pos:
            self.log("No positions to plot in 3D")
            return

        # Draw edges
        self._draw_graph_edges(self.graph_view_3d, graph, pos, is_3d=True)
        
        # Draw nodes
        self._draw_graph_nodes(self.graph_view_3d, graph, pos, is_3d=True)
        
        # Set view limits (camera position)
        self._set_view_limits(self.graph_view_3d, pos, is_3d=True)
        
        # Switch to 3D graph tab
        self.tab_widget.setCurrentWidget(self.graph_view_3d)
    ...
    def _visualize_clustering_results(self, view, clustering_results: Dict[str, Any], is_3d: bool = True):
        """
        Visualize clustering results on a given GLViewWidget (3D).
        """
        if not clustering_results or not self.current_graph:
            self.log(f"Clustering results not available or missing graph object for 3D visualization.")
            return

        view.clear()
        pos = self._get_node_positions(self.current_graph)
        
        self._draw_base_chassis_graph(view, pos)
        self._draw_clusters_and_centroids(view, pos, clustering_results)
        self._draw_overlay_legend(view, clustering_results)

        self._set_view_limits(view, pos, is_3d=True)
        self.tab_widget.setCurrentWidget(self.cluster_view_3d)

    def _draw_overlay_legend(self, view: gl.GLViewWidget, clustering_results: Dict[str, Any]):
        """Draws a legend directly on the GLViewWidget as an overlay."""
        legend_items = []

        # Chassis Nodes
        chassis_style = self._get_node_style("C_dummy_node", type_name_override="chassis")
        chassis_color = chassis_style["brush"].color()
        legend_items.append(("Chassis Node", chassis_color))

        # I/O Nodes
        io_style = self._get_node_style("IO_dummy_node", type_name_override="io")
        io_color = io_style["brush"].color()
        legend_items.append(("I/O Node", io_color))
        ...
        for i, (cluster_id, cluster_data) in enumerate(clusters.items()):
            if cluster_id != "can_bus":
                ...
                legend_items.append((f"Extender {cluster_id.split('_')[1]}", color))
        
        # CAN Bus
        if "can_bus" in clusters:
            can_bus_color = QColor(255, 0, 0) # Red for CAN bus
            legend_items.append(("CAN Bus", can_bus_color))

        # Create a QGraphicsView to act as an overlay
        legend_overlay_view = QGraphicsView(view)
        legend_overlay_view.setFixedSize(200, len(legend_items) * 25 + 30) # Adjust size based on content
        legend_overlay_view.setStyleSheet("background-color: rgba(255, 255, 255, 180); border: 1px solid black;")
        legend_overlay_view.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        legend_overlay_view.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        
        # Position the legend in the top-left corner of the GLViewWidget
        legend_overlay_view.move(10, 10) 

        legend_scene = QGraphicsScene(legend_overlay_view)
        legend_overlay_view.setScene(legend_scene)
        ...
        for text, color in legend_items:
            ...
            color_rect.setBrush(color)
            color_rect.setPen(QColor(Qt.black))
            legend_scene.addItem(color_rect)

            # Text label
            text_item = QGraphicsTextItem(text)
            text_item.setFont(QFont("Arial", 10))
            text_item.setPos(25, y_offset)
            legend_scene.addItem(text_item)
            
            y_offset += 25 

        # Make the overlay visible
        legend_overlay_view.show()

    def _draw_base_chassis_graph(self, view, pos):
        """Draws the base chassis graph (nodes and edges) lightly."""
        ...
        # Ensure base chassis edges and nodes are drawn opaquely
        self._draw_graph_edges(view, chassis_graph, pos, alpha=1.0, name="Chassis Edge", is_3d=True) 
        self._draw_graph_nodes(view, chassis_graph, pos, alpha=0.1, is_3d=True) 

    def _draw_clusters_and_centroids(self, view, pos, clustering_results: Dict[str, Any]):
        """Draws I/O nodes, wiring paths, and centroids for each cluster."""
        clusters = clustering_results.get("clusters", {})
        colors = self._get_cluster_colors()
        ...
        for i, (cluster_id, cluster_data) in enumerate(clusters.items()):
            ...
            self._draw_io_nodes_for_cluster(view, pos, cluster_data, color)
            self._draw_wiring_paths_for_cluster(view, pos, cluster_data, color)
            self._draw_centroid_node(view, cluster_id, cluster_data, color)

    def _draw_io_nodes_for_cluster(self, view, pos, cluster_data: Dict[str, Any], color: QColor):
        """Draws I/O nodes belonging to a specific cluster."""
        io_nodes_in_cluster = cluster_data.get("io_nodes", [])
        
        # Apply z_offset for IO nodes (which are not junctions or extenders, so z_offset=0)
        io_points_with_z_offset = []
        for node in io_nodes_in_cluster:
            if node in pos:
                x, y, z = pos[node]
                io_points_with_z_offset.append((x, y, z + 0.0)) 
        
        if io_points_with_z_offset:
            points = np.array(io_points_with_z_offset)
            style = self._get_node_style("", type_name_override="io")
            size_3d = style.get("size", 7) * self.gl_node_size_multiplier
            rgba = color.getRgbF()[:3] + (1.0,)
            for x, y, z in points:
                item = self._make_solid_item("sphere", (x, y, z), size_3d, rgba, z_offset=0.0)
                view.addItem(item)

    def _draw_wiring_paths_for_cluster(self, view, pos, cluster_data: Dict[str, Any], color: QColor):
        """Draws wiring paths from I/O nodes to the centroid for a cluster."""
        ...
        for io_node, path_data in wiring_paths.items():
            ...
            if centroid_info and "pos" in centroid_info:
                ...
                if len(vis_path_points) > 1:
                    points_3d = np.array(vis_path_points)
                    line_item = gl.GLLinePlotItem(pos=points_3d, color=color.getRgbF(), width=2.5, antialias=True)
                    view.addItem(line_item)

    def _draw_centroid_node(self, view, cluster_id: str, cluster_data: Dict[str, Any], color: QColor):
        """Draws the I/O extender (centroid) node for a cluster."""
        centroid_info = cluster_data.get("centroid")
        if centroid_info and "pos" in centroid_info and cluster_id != "can_bus":
            cx, cy, cz = centroid_info["pos"]
            size_3d_centroid = 12 * self.gl_node_size_multiplier
            # Determine the symbol for I/O extender nodes from node_definitions
            io_extender_style = self._get_node_style("E_dummy_node", type_name_override="io_extender")
            centroid_shape_type = io_extender_style.get("shape_3d", "sphere") 
            
            # I/O extenders should be in front
            z_offset = 1.0 

            self.logger.debug(f"Drawing centroid {cluster_id} at {cx, cy, cz} with size {size_3d_centroid}, shape: {centroid_shape_type}, z_offset: {z_offset}")
            opaque_color = color.getRgbF()[:3] + (1.0,)
            item = self._make_solid_item(centroid_shape_type, (cx, cy, cz),
                                         size_3d_centroid, opaque_color, z_offset=z_offset)
            view.addItem(item)

    def _visualize_hpc_wiring(self, is_3d: bool = True):
        """Visualize HPC wiring results."""
        view = self.hpc_view_3d

        if not self.hpc_results or not self.current_graph:
            self.log(f"HPC results not available or missing graph object for 3D visualization.")
            return

        view.clear()
        pos = self._get_node_positions(self.current_graph)
        
        # Draw base graph opaquely
        self._draw_graph_edges(view, self.current_graph, pos, alpha=1.0, name="Chassis Edge", is_3d=True)
        self._draw_graph_nodes(view, self.current_graph, pos, alpha=1.0, is_3d=True)
        ...
        for io_node, path_data in paths.items():
            path = path_data.get("path", [])
            if len(path) > 1:
                points_3d = np.array([pos[node] for node in path if node in pos])
                if len(points_3d) > 1:
                    line_item = gl.GLLinePlotItem(pos=points_3d, color=(1, 0, 0, 1), width=3, antialias=True)
                    view.addItem(line_item)

        self._set_view_limits(view, pos, is_3d=True)
        self.tab_widget.setCurrentWidget(self.hpc_view_3d)

    # ==== UTILITY FUNCTIONS ====

    def generate_clustering_plot_for_report(self, clustering_results: Dict[str, Any], title: str, is_3d: bool = True) -> bytes:
        """
        ...
        """
        temp_view = gl.GLViewWidget()
        self._setup_gl_view(temp_view, title)
        
        # Use the refactored visualization method to draw on the temporary widget
        self._visualize_clustering_results(temp_view, clustering_results, is_3d=True)
        
        # Export the contents of the temporary widget to image bytes
        return self._export_plot_to_image_bytes(temp_view)

    def _export_plot_to_image_bytes(self, plot_widget) -> bytes:
        """
        Exports a pyqtgraph GLViewWidget (3D) to an in-memory PNG and returns the bytes.
        
        This allows non-GUI modules like the report generator to access plot
        visualizations without directly handling GUI objects.

        Args:
            plot_widget: The pyqtgraph widget to export (gl.GLViewWidget).

        Returns:
            The PNG image data as a byte string.
        """
        if isinstance(plot_widget, gl.GLViewWidget):
            # For GLViewWidget, grabFramebuffer is used
            pixmap = plot_widget.grabFramebuffer()
        else:
            raise TypeError("Unsupported plot_widget type for export. Expected gl.GLViewWidget.")
        
        buffer = QBuffer()
        buffer.open(QIODevice.ReadWrite)
        # Save the pixmap to the buffer in PNG format. PNG is chosen for its lossless compression, which is ideal for line art like graphs
    ...
    def _get_node_positions(self, graph) -> Dict[str, Tuple[float, float, float]]:
        """Extract node positions from graph, ensuring 3D coordinates."""
        pos = {}
        for node, data in graph.nodes(data=True):
            node_pos = data.get('pos')
            if node_pos and len(node_pos) >= 2:
                try:
                    x, y = float(node_pos[0]), float(node_pos[1])
                    z = float(node_pos[2]) if len(node_pos) > 2 else 0.0
                    pos[node] = (x, y, z)
                except (ValueError, TypeError):
                    self.logger.warning(f"Could not parse 3D position for node {node}: {node_pos}")
                    continue
        return pos

    def _draw_graph_edges(self, view, graph, pos, alpha=1.0, name=None, is_3d: bool = False):
        """Draw graph edges."""
        if is_3d:
            points = []
            ...
            if points:
                ...
                # Ensure edges are fully opaque (R, G, B, 1.0)
                opaque_edge_color = (0, 0, 0, 1.0)
                line_item = gl.GLLinePlotItem(pos=points, color=opaque_edge_color, width=1, antialias=True, glOptions='opaque')
                view.addItem(line_item)
        else:
            xs, ys = [], []
            for u, v in graph.edges():
                if u in pos and v in pos:
                    # Unpack only X and Y for 2D view
                    xu, yu, _ = pos[u]
                    xv, yv, _ = pos[v]
    ...
    def _get_node_style(self, node_name: str, type_name_override: str = None) -> Dict[str, Any]:
        ...
        # Store the actual type name
        actual_node_type_name = "default"
        if not type_to_find:
            for name, info in node_types.items():
                for prefix in info.get("prefixes", []):
                    if node_name.startswith(prefix):
                        type_to_find = name
                        actual_node_type_name = name
                        break
                if type_to_find:
                    break
        else:
            actual_node_type_name = type_to_find
        ...
        return {
            ...
            "brush": pg.mkBrush(color=rgb + (255,)), # Explicitly set alpha to 255 (opaque)
            "pen": pg.mkPen(color=tuple(c * 0.6 for c in rgb)),
            "size": type_info.get("size", 6),
            "shape_3d": type_info.get("shape_3d", "sphere"), # Get 3D shape directly
            "node_type_name": actual_node_type_name # Add node type name
        }
        
    def _draw_graph_nodes(self, view, graph, pos, alpha=1.0, is_3d: bool = False):
        """Draw graph nodes based on config-driven styles."""
        if not is_3d:
            self.log("2D node drawing is deprecated and not supported.")
            return

        nodes_by_style = self._group_nodes_by_style(graph, pos)

        for style_key, group in nodes_by_style.items():
            points = np.array(group["points"])
            style = group["style"]
            self._draw_3d_nodes_for_style(view, points, style)

    def _group_nodes_by_style(self, graph, pos) -> Dict[str, Any]:
        """Helper to group nodes by their determined style."""
        nodes_by_style = {}
        for node, data in graph.nodes(data=True):
            if node not in pos:
                continue
        ...
        return nodes_by_style

    def _draw_3d_nodes_for_style(self, view, points: np.ndarray, style: Dict[str, Any]):
        """Helper to draw 3D nodes for a given style."""
        size_3d = style["size"] * self.gl_node_size_multiplier
        shape_type = style.get("shape_3d", "sphere")
        color_rgb = style["brush"].color().getRgbF()
        node_type_name = style.get("node_type_name", "default") # Get node type name

        z_offset = 0.0
        if node_type_name in ["junction", "Wire junction"]:
            z_offset = -1.0 # Draw junction nodes slightly behind
        elif node_type_name == "io_extender":
            z_offset = 1.0 # Draw I/O extenders slightly in front
        # Other nodes (io, hpc, sensor, actuator, power, default) will have z_offset = 0.0
        
        for point in points:
            self.logger.debug(f"Drawing node at {point} with size {size_3d}, shape: {shape_type}, type: {node_type_name}, z_offset: {z_offset}")
            rgba = color_rgb[:3] + (1.0,)  # force full opacity
            item = self._make_solid_item(shape_type, (point[0], point[1], point[2]),
                                         size_3d, rgba, z_offset=z_offset)
            view.addItem(item)

    def _set_view_limits(self, view, pos, is_3d: bool = False):
        """Set appropriate view limits for the plot."""
        if not pos:
            if is_3d:
                view.setCameraPosition(distance=10000, elevation=30, azimuth=45)
            else:
        ...
        points_array = np.array(list(pos.values()))
        
        if is_3d:
            # For 3D, set camera position based on data extent
            ...
            # Get default elevation and azimuth from config
            elevation = self.config.get("gui.3d_view_elevation", 30)
            azimuth = self.config.get("gui.3d_view_azimuth", 45)

            view.setCameraPosition(pos=pg.Vector(center_x, center_y, center_z),
    ...
    def load_chassis_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Load Chassis Graph", data_dir, "JSON Files (*.json);;All Files (*)"
        )
        if file_path:
    ...
    def load_io_file(self):
        ...
        data_dir = self.config.get("paths.data_dir", "./data")
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Load I/O Coordinates", data_dir, "CSV Files (*.csv);;All Files (*)"
        )
        if file_path:
